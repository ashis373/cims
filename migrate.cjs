...
files.forEach((file) => {
  ...
  content = content.replace(
    /const\s+pathname\s*=\s*useRouterState\(\{\s*select:\s*\([^)]+\)\s*=>\s*[^.]+\.location\.pathname\s*\}\);/g,
  ...
  content = content.replace(
    /navigate\(\{\s*to:\s*["']\/candidates\/\$id["'],\s*params:\s*\{\s*id:\s*([^}]+)\s*\}\s*\}\)/g,
    "navigate(`/candidates/${$1}`)",
  );
  content = content.replace(
    /navigate\(\{\s*to:\s*["']\/candidates\/\$id["'],\s*params:\s*\{\s*id\s*\}\s*\}\)/g,
  ...
  content = content.replace(
    /to=["']\/candidates\/\$id["']\s+params=\{\{\s*id:\s*([^}]+)\s*\}\}/g,
    "to={`/candidates/${$1}`}",
  );

  // 5. Remove createFileRoute
  content = content.replace(
    /export\s+const\s+Route\s*=\s*createFileRoute\([^)]+\)\(\{\s*head:\s*\([^)]+\)\s*=>\s*\(\{[^}]+\}\),\s*component:\s*[a-zA-Z0-9_]+,\s*\}\);?/g,
    "",
  );
  content = content.replace(
    /export\s+const\s+Route\s*=\s*createFileRoute\([^)]+\)\(\{\s*component:\s*[a-zA-Z0-9_]+,\s*\}\);?/g,
    "",
  );
