...
import { Plus, Users, CalendarCheck, Trophy, XCircle, Download, TrendingUp, Star, Mail, CheckCircle, Ban, UserX, User, Send, ArrowUp } from "lucide-react";
...
function Stat({
  ...
  return (
    <motion.div variants={item}>
      <Card className={cn("p-5 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl relative overflow-hidden group border-t-[3px] h-full", borderTone)}>
...
function Dashboard() {
  ...
  const counts = useMemo(() => {
    ...
    const isInactive = (st: Stage) => ["Rejected", "Offer Declined", "No Show", "Offer Expired"].includes(st);
  ...
  const quickViews = [
    { label: "New Candidates", filter: "New Applicant", icon: Users, color: "text-blue-600 bg-blue-50 border-blue-100" },
    { label: "Interviews Today", filter: "Interview Scheduled", icon: CalendarCheck, color: "text-violet-600 bg-violet-50 border-violet-100" },
    { label: "Pending Offers", filter: "Offer Released", icon: Send, color: "text-amber-600 bg-amber-50 border-amber-100" },
    { label: "Offer Accepted", filter: "Offer Accepted", icon: CheckCircle, color: "text-emerald-600 bg-emerald-50 border-emerald-100" },
    { label: "Joined", filter: "Joined", icon: User, color: "text-teal-600 bg-teal-50 border-teal-100" },
    { label: "Rejected", filter: "Rejected", icon: XCircle, color: "text-rose-600 bg-rose-50 border-rose-100" },
    { label: "Blacklisted", filter: "Blacklisted", icon: Ban, color: "text-slate-600 bg-slate-100 border-slate-200" },
    { label: "Future Pool", filter: "On Hold", icon: Users, color: "text-indigo-600 bg-indigo-50 border-indigo-100" },
  ];
    ...
    <motion.div initial="hidden" animate="show" variants={container} className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => setExportOpen(true)} className="bg-white text-slate-700 shadow-sm border-slate-200 text-xs h-9">
            <Download className="mr-2 h-3.5 w-3.5" />
            Export
          </Button>
          <Button asChild className="bg-blue-600 hover:bg-blue-700 text-white shadow-sm text-xs h-9">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        <Stat label="Total Candidates" value={counts.total} icon={Users} tone="bg-blue-50 text-blue-600" borderTone="border-blue-500" />
        <Stat label="Active Candidates" value={counts.active} icon={User} tone="bg-emerald-50 text-emerald-600" borderTone="border-emerald-500" />
        <Stat label="Interview Scheduled" value={counts.scheduled} icon={CalendarCheck} tone="bg-violet-50 text-violet-600" borderTone="border-violet-500" />
        <Stat label="Selected" value={counts.selected} icon={Star} tone="bg-amber-50 text-amber-500" borderTone="border-amber-500" />
        <Stat label="Offers Released" value={counts.offersReleased} icon={Mail} tone="bg-rose-50 text-rose-500" borderTone="border-rose-500" />
        
        <Stat label="Offers Accepted" value={counts.offersAccepted} icon={CheckCircle} tone="bg-emerald-50 text-emerald-500" borderTone="border-emerald-500" />
        <Stat label="Joined" value={counts.joined} icon={User} tone="bg-purple-50 text-purple-600" borderTone="border-purple-500" />
        <Stat label="Rejected" value={counts.rejected} icon={XCircle} tone="bg-red-50 text-red-500" borderTone="border-red-500" />
        <Stat label="Blacklisted" value={counts.blacklisted} icon={Ban} tone="bg-slate-100 text-slate-600" borderTone="border-slate-300" />
        <Stat label="No Join" value={counts.noShow} icon={UserX} tone="bg-orange-50 text-orange-500" borderTone="border-orange-500" />
      </div>
          ...
          <Card className="p-6 bg-white border-border/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl h-full flex flex-col">
            ...
            <div className="flex gap-8 flex-1">
              ...
              <div className="w-1/2 flex flex-col justify-center space-y-0.5">
                {funnelData.map((d) => (
                  <div key={d.stage} className="flex items-center justify-between text-[11px] font-medium py-[3px]">
              ...
              {recent.map((c) => (
                ...
                >
                  <div className="flex items-center gap-3.5">
                    <div className={cn("flex h-10 w-10 items-center justify-center rounded-full text-[11px] font-bold uppercase transition-colors", STAGE_COLORS[c.stage])}>
            ...
            {quickViews.map((q) => {
              ...
              return (
                <Link 
                  key={q.label} 
                  to="/candidates"
                  className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] font-bold hover:opacity-80 transition-opacity", q.color)}
                >
