...
import { Download, TrendingUp, Users, Building2, Share2, Briefcase, XOctagon, Target, Activity, CalendarDays, Award, ChevronRight, ArrowUpRight } from "lucide-react";
...
function StatCard({ title, value, subtitle, icon: Icon, tone, borderTone }: any) {
  return (
    <Card className={cn("p-6 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl relative overflow-hidden group border-t-[3px]", borderTone)}>
      ...
      <div className="flex items-start justify-between relative z-10">
        <div>
          <div className="text-[12px] font-bold text-slate-500 uppercase tracking-wider mb-2">{title}</div>
...
function ReportsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const recruiterPerf = Object.entries(recruiterStats).map(([name, stats]) => ({
      name,
      ...stats,
    ...
    const sourceChartData = SOURCES.map(s => ({ name: s, value: bySource[s] })).filter(s => s.value > 0);

    return { monthlyData, conversion, recruiterPerf, rejections, stageChartData, deptChartData, sourceChartData };
  }, [candidates]);
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Button variant="outline" className="h-10 px-5 rounded-xl font-bold text-[13px] border-slate-200 text-slate-700 bg-white" asChild>
             <Link to="/pipeline"><Activity className="h-4 w-4 mr-2 text-blue-600" /> Pipeline View</Link>
          </Button>
          <Button
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <Card className="xl:col-span-2 p-5 bg-white border-border/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl flex flex-col h-[300px]">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-[14px] font-bold text-slate-900 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-blue-600" /> Pipeline Distribution</h3>
            </div>
          </div>
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.stageChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
        ...
        <Card className="xl:col-span-1 p-5 bg-slate-900 border-none shadow-xl rounded-3xl text-white flex flex-col relative overflow-hidden h-[300px]">
          ...
          <div className="relative z-10 mb-4">
            <h3 className="text-[14px] font-bold flex items-center gap-2"><Activity className="h-4 w-4 text-blue-400" /> Application Volume</h3>
          </div>
          <div className="flex-1 min-h-0 relative z-10 -mx-2 -mb-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.monthlyData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                <defs>
                ...
                <Area type="monotone" dataKey="value" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorCount)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="relative z-10 flex justify-between items-end mt-4 px-2">
            {stats.monthlyData.map(m => (
              <span key={m.name} className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{m.name}</span>
            ))}
          </div>
        </Card>
        ...
        <Card className="xl:col-span-1 p-5 bg-white border-border/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl flex flex-col h-[300px]">
          <div className="mb-2">
            <h3 className="text-[14px] font-bold text-slate-900 flex items-center gap-2"><Share2 className="h-4 w-4 text-purple-600" /> Sources</h3>
          </div>
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
          ...
          <div className="space-y-1.5 mt-2 overflow-y-auto max-h-[80px] scrollbar-thin">
            {stats.sourceChartData.sort((a,b)=>b.value-a.value).map((s, i) => (
               <div key={s.name} className="flex items-center justify-between">
                 <div className="flex items-center gap-2">
                   <div className="h-2 w-2 rounded-sm" style={{ backgroundColor: COLORS[(i + 2) % COLORS.length] }} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <Card className="xl:col-span-2 p-5 bg-white border-border/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl flex flex-col h-[300px]">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h3 className="text-[14px] font-bold text-slate-900 flex items-center gap-2"><Briefcase className="h-4 w-4 text-emerald-600" /> Recruiter Performance</h3>
            </div>
            <Button variant="link" className="text-[10px] h-6 px-0 font-bold text-blue-600">View Details <ArrowUpRight className="h-3 w-3 ml-1" /></Button>
          </div>
            ...
            <table className="w-full text-left text-[10px]">
              ...
              <tbody className="divide-y divide-slate-50 text-slate-900 font-semibold">
                {stats.recruiterPerf.map((r, i) => (
                  <tr key={r.name} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <img src={`https://api.dicebear.com/7.x/notionists/svg?seed=${r.name}`} alt="" className="h-6 w-6 rounded-full bg-slate-100 border border-slate-200" />
                        <span>{r.name}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-center font-bold text-emerald-600">{r.hired}</td>
                    <td className="px-3 py-2.5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                         <div className="w-12 h-1 bg-slate-100 rounded-full overflow-hidden">
                           <div className="h-full bg-blue-500 rounded-full" style={{ width: `${r.conversion}%` }} />
                         </div>
                ...
                {stats.recruiterPerf.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-400">No recruiter data available</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <Card className="xl:col-span-1 p-5 bg-white border-border/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl flex flex-col h-[300px]">
          <div className="mb-4">
            <h3 className="text-[14px] font-bold text-slate-900 flex items-center gap-2"><Building2 className="h-4 w-4 text-indigo-600" /> Departments</h3>
          </div>
          <div className="space-y-3.5 overflow-y-auto scrollbar-thin pr-2 flex-1">
            {stats.deptChartData.sort((a,b)=>b.value-a.value).map((d, i) => {
        ...
        <Card className="xl:col-span-1 p-5 bg-white border-border/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl flex flex-col h-[300px]">
          <div className="mb-4">
            <h3 className="text-[14px] font-bold text-slate-900 flex items-center gap-2"><XOctagon className="h-4 w-4 text-red-500" /> Rejections</h3>
          </div>
          <div className="space-y-2.5 overflow-y-auto scrollbar-thin flex-1">
            {stats.rejections.length > 0 ? stats.rejections.map((r, i) => (
              <div key={r.name} className="flex items-center p-2.5 bg-slate-50 rounded-xl border border-slate-100">
