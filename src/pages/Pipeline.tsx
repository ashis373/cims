...
import { 
  Search, Undo2, Filter, LayoutGrid, Plus, 
  Users, Activity, CalendarCheck, CalendarDays, Trophy,
  ArrowUp
} from "lucide-react";
...
function Stat({ label, value, icon: Icon, tone, percent, borderTone }: any) {
  return (
    <Card className={cn("p-5 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] rounded-3xl relative overflow-hidden group border-t-[3px] flex-1 min-w-[180px]", borderTone)}>
      ...
      <div className="flex items-start justify-between relative z-10">
        <div>
          <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">{label}</div>
...
const mockVelocityData = [
  { date: "May 1", value: 10 }, { date: "May 5", value: 30 }, 
  { date: "May 8", value: 25 }, { date: "May 12", value: 45 },
  { date: "May 15", value: 40 }, { date: "May 20", value: 60 },
  { date: "May 22", value: 58 }, { date: "May 25", value: 75 },
...
function Pipeline() {
  ...
  const counts = useMemo(() => {
    ...
    const isInactive = (st: string) => ["Rejected", "Offer Declined", "No Show", "Offer Expired"].includes(st);
  ...
  const analytics = useMemo(() => {
    let newApp = 0, short = 0, intSched = 0, intComp = 0, offerRel = 0, other = 0;
    ...
    const overview = [
      { label: "New Applicant", value: newApp, pct: ((newApp/total)*100).toFixed(1)+'%', color: "bg-blue-500", hex: "#3b82f6" },
      { label: "Shortlisted", value: short, pct: ((short/total)*100).toFixed(1)+'%', color: "bg-emerald-500", hex: "#10b981" },
      { label: "Interview Scheduled", value: intSched, pct: ((intSched/total)*100).toFixed(1)+'%', color: "bg-purple-500", hex: "#8b5cf6" },
      { label: "Interview Completed", value: intComp, pct: ((intComp/total)*100).toFixed(1)+'%', color: "bg-amber-500", hex: "#f59e0b" },
      { label: "Offer Released", value: offerRel, pct: ((offerRel/total)*100).toFixed(1)+'%', color: "bg-yellow-500", hex: "#eab308" },
      { label: "Other / Closed", value: other, pct: ((other/total)*100).toFixed(1)+'%', color: "bg-slate-200", hex: "#e2e8f0" },
    ];
    ...
    const sortedSources = Object.entries(sourceCounts)
      ...
      .map(([label, val], i) => {
        ...
        const bgColors = ["bg-blue-500", "bg-emerald-500", "bg-purple-500", "bg-amber-500", "bg-yellow-500"];
        return {
          label,
          value: val,
    ...
    if (recentActivity.length === 0) {
      recentActivity.push({ msg: "No recent activity", time: "Just now", icon: Activity, color: "text-slate-500 bg-slate-50 border-slate-100" });
    }

    const hired = candidates.filter(c => c.stage === "Joined");
    let avgHireTime = 18;
    if (hired.length > 0) {
      const times = hired.map(c => Math.max(1, (+new Date(c.updatedAt) - +new Date(c.appliedAt)) / (1000 * 3600 * 24)));
    ...
    const rejects = candidates.filter(c => ["Rejected", "Offer Declined", "No Show"].includes(c.stage)).length;
    const dropOff = Math.round((rejects / total) * 100);

    return {
      overview,
      topSkills: sortedSkills.length > 0 ? sortedSkills : [{label: "No Data", pct: 0, color: "bg-slate-200"}],
      sortedSources: sortedSources.length > 0 ? sortedSources : [{label: "No Data", value: 1, pct: "0%", hex: "#e2e8f0", color: "bg-slate-200"}],
      recentActivity,
      stats: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">Drag cards between columns to update stage.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" size="sm" onClick={undo} disabled={!canUndo} className="h-9 w-9 p-0 bg-white shadow-sm border-slate-200">
            <Undo2 className="h-4 w-4 text-slate-600" />
          </Button>
          <Button variant="outline" className="h-9 text-xs bg-white shadow-sm border-slate-200 text-slate-700 font-semibold">
          ...
          <Button asChild className="h-9 text-xs bg-blue-600 hover:bg-blue-700 text-white shadow-sm font-semibold rounded-lg">
      ...
      <div className="flex flex-wrap gap-4">
        <Stat label="Total Candidates" value={counts.total} icon={Users} tone="bg-blue-50 text-blue-600" percent={12} borderTone="border-blue-500" />
        <Stat label="Active Pipeline" value={counts.active} icon={Activity} tone="bg-emerald-50 text-emerald-600" percent={8} borderTone="border-emerald-500" />
        <Stat label="Interviews Today" value={counts.scheduled} icon={CalendarCheck} tone="bg-purple-50 text-purple-600" percent={15} borderTone="border-purple-500" />
        <Stat label="Offers Pending" value={counts.offersPending} icon={CalendarDays} tone="bg-orange-50 text-orange-500" percent={5} borderTone="border-orange-500" />
        <Stat label="Hired This Month" value={counts.hired} icon={Trophy} tone="bg-teal-50 text-teal-600" percent={20} borderTone="border-teal-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="col-span-1 lg:col-span-2 p-6 bg-white rounded-3xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border-border/50 flex flex-col">
          ...
          <div className="h-[140px] w-full mb-6">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={mockVelocityData} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                <defs>
          ...
          <div className="grid grid-cols-4 gap-4 mt-auto">
            {[
              ...
              <div key={s.label} className="border border-slate-100 rounded-xl p-3 flex flex-col justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-10">
        <Card className="p-6 bg-white rounded-3xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border-border/50">
          ...
          <div className="space-y-5">
            ...
            <div className="pt-2 text-center">
              <Button variant="link" className="text-xs font-bold text-blue-600">View full report →</Button>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6 bg-white rounded-3xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border-border/50 flex flex-col">
            ...
            {analytics.recentActivity.map((act, i) => {
              ...
              return (
                <div key={i} className="flex gap-3 items-start">
                  <div className={cn("flex h-7 w-7 shrink-0 items-center justify-center rounded-full border", act.color)}>
          ...
          <div className="pt-2 text-center">
            <Button variant="link" className="text-xs font-bold text-blue-600">View all activity →</Button>
          </div>
        </Card>
      </div>
    </div>
