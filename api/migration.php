...
try {
    ...
    $conn->exec("
    CREATE TABLE IF NOT EXISTS cims_candidates_normalized (
    ...
    // 3. JOB OPENINGS
    $conn->exec("
    CREATE TABLE IF NOT EXISTS cims_job_openings (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        createdBy INT,
        createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (createdBy) REFERENCES cims_users(id) ON DELETE SET NULL
    )
    ");

    // 4. APPLICATIONS
    $conn->exec("
    CREATE TABLE IF NOT EXISTS cims_applications (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        FOREIGN KEY (candidate_id) REFERENCES cims_candidates_normalized(id) ON DELETE CASCADE,
        FOREIGN KEY (job_id) REFERENCES cims_job_openings(id) ON DELETE SET NULL,
    ...
    $conn->exec("
    CREATE TABLE IF NOT EXISTS cims_candidate_interviews (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        FOREIGN KEY (application_id) REFERENCES cims_applications(id) ON DELETE CASCADE,
    ...
    // 6. CANDIDATE OFFERS
    $conn->exec("
    CREATE TABLE IF NOT EXISTS cims_candidate_offers (
        id INT AUTO_INCREMENT PRIMARY KEY,
        application_id INT NOT NULL,
    ...
    $conn->exec("
    CREATE TABLE IF NOT EXISTS cims_candidate_rejections (
        id INT AUTO_INCREMENT PRIMARY KEY,
        candidate_id VARCHAR(50) NOT NULL,
    ...
    // 8. CANDIDATE HISTORY
    $conn->exec("
    CREATE TABLE IF NOT EXISTS cims_candidate_history (
        id INT AUTO_INCREMENT PRIMARY KEY,
        candidate_id VARCHAR(50) NOT NULL,
        ...
        userId INT NULL,
        FOREIGN KEY (candidate_id) REFERENCES cims_candidates_normalized(id) ON DELETE CASCADE,
        FOREIGN KEY (userId) REFERENCES cims_users(id) ON DELETE SET NULL
    )
    ");

    // Data Migration
    $conn->exec("
    INSERT IGNORE INTO cims_candidates_normalized (
    ...
    FROM cims_candidates WHERE isBlacklisted = 1 AND blacklistReason IS NOT NULL;
    ");

    $conn->exec("
    INSERT IGNORE INTO cims_candidate_rejections (candidate_id, type, reason, recordedAt)
    SELECT id, 'Rejected', rejectionReason, rejectionDate
    FROM cims_candidates WHERE stage = 'Rejected' AND rejectionReason IS NOT NULL;
    ");

    $conn->exec("
    INSERT IGNORE INTO cims_candidate_rejections (candidate_id, type, reason)
    SELECT id, 'No-Join', noJoinReason
    FROM cims_candidates WHERE stage = 'No Show' AND noJoinReason IS NOT NULL;
    ");
